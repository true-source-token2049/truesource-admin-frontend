...
const MAX_BLOCKS_TO_SCAN = 1000; // Scan last ~1000 blocks (enough for 4-5 recent transactions)
...
export async function GET(request) {
  try {
    ...
    // Query Transfer events for this token in chunks
    ...
    let allEvents = [];
    try {
      // Query in chunks to work within Alchemy free tier limits
      ...
      // If query fails, at least return current owner
    ...
    // Extract all owners in order (excluding zero address for minting)
    const allOwners = [];
    const seenOwners = new Set();

    transfers.forEach(transfer => {
      // Add 'to' address as owner if not seen
    ...
    // Previous owners = all owners except current
