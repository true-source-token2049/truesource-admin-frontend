...
export const WalletProvider = ({ children }) => {
  ...
  const connectWallet = useCallback(async () => {
    ...
    try {
      ...
      if (chainIdHex === targetChainId) {
        ...
        setError(`Please switch to the Sepolia network. You are currently on ${network.name}.`);
      }
    } catch (err) {
  ...
  const handleChainChanged = useCallback((chainId) => {
    ...
    if (chainIdLower === targetChainId) {
      ...
      setIsCorrectNetwork(false);
      setError(`Please switch to the Sepolia network. You are currently on chain ${chainId}.`);
    }
  }, []);
